...
class CommentCreate(APITestCase):
    ...
    def setUp(self):
        ...
        self.password = "12345"
        self.post = Post.objects.create(title="title", content="testicerik")
        self.user = User.objects.create_user(username=self.username, password=self.password)
        self.parent_comment = Comment.objects.create(content="test yorum", user=self.user, post=self.post)
        self.test_jwt_authentication()

    def test_jwt_authentication(self):
        response = self.client.post(self.login_url, data={"username": "admintest", "password": "12345"})
...
class CommentUpdateDeleteTest(APITestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(username=self.username, password=self.password)
        self.user2 = User.objects.create_user(username="admintest1", password=self.password)
    ...
    def test_jwt_authentication(self, username="admintest", password="12345"):
    ...
    def test_delete_other_user(self):
        self.test_jwt_authentication("admintest1")
        response = self.client.delete(self.url)
